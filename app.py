"""
Dashboard Econométrico — Región del Bajío (VERSIÓN AVANZADA)
============================================================
Variables: Empleo Manufacturero, IED, Actividad Industrial, Exportaciones
Mejoras: Filtros 2020, Scatter interactivo con OLS, Econometría en Tasas de Crecimiento
"""
...
# ══════════════════════════════════════════════
def fetch_inegi_serie(indicador: str, fuente: str = "BIE", geo: str = "00") -> pd.DataFrame:
    url = (
            ...
            for s in obs:
                if s["OBS_VALUE"] not in (None, "", "N/A"):
                    try:
                        ...
                        pass
        if not all_rows: raise ValueError("Serie vacía")
...
def simular_empleo_nacional() -> pd.DataFrame:
    # Simulamos el total nacional para el KPI (aprox 4.5M empleos)
    rows = []
    for yr in YEARS:
        for q in range(1,5):
            if yr==2025 and q>1: break
...
_cache = {"panel": None, "nacional": None, "geojson": None, "fecha": None, "lock": threading.Lock()}

def get_datos():
    hoy = date.today()
    if _cache["panel"] is not None and _cache["fecha"] == hoy:
        return _cache["panel"], _cache["nacional"], _cache["geojson"]
    with _cache["lock"]:
        if _cache["panel"] is not None and _cache["fecha"] == hoy:
            return _cache["panel"], _cache["nacional"], _cache["geojson"]
        ...
        df_nac  = simular_empleo_nacional()
        geojson = cargar_geojson()
        _cache["panel"] = panel; _cache["nacional"] = df_nac; _cache["geojson"] = geojson; _cache["fecha"] = hoy
    return _cache["panel"], _cache["nacional"], _cache["geojson"]
...
# ══════════════════════════════════════════════
# MÓDULO ECONOMÉTRICO (AHORA EN TASAS DE CRECIMIENTO)
# ══════════════════════════════════════════════
def calcular_econometria(df: pd.DataFrame, vars_x: list):
    if len(vars_x) == 0: return None
    
    cols = ["Empleo_Manufacturero"] + vars_x
    sub = df[["Estado","Año","Trimestre"] + cols].dropna().copy()
    if len(sub) < 20: return None
    
    # 🌟 MEJORA: Transformar a tasas de crecimiento para un modelo más realista
    for c in cols:
...
def fig_scatter_avanzado(df, var_x, var_y, estados, mostrar_tendencia):
    ...
    fig = px.scatter(
        ...
        trendline="ols" if mostrar_tendencia else None, trendline_scope="overall"
    )
    
    fig.update_traces(marker=dict(size=8, opacity=0.7))
    fig.update_layout(**{**PLOT_LAYOUT,"xaxis":dict(title=VAR_LABEL[var_x],gridcolor="#EEE",tickformat=","),
        "yaxis":dict(title=VAR_LABEL[var_y],gridcolor="#EEE"),"height":300})
    return fig
...
app.layout = html.Div(
    ...
    html.Div(style={**STYLE_CARD,"marginBottom":"16px"},children=[
        html.Div(style={"display":"flex","flexWrap":"wrap","gap":"20px","alignItems":"flex-start"},children=[
            html.Div([
                html.P("Estados",style={"fontSize":"11px","color":TEXT_SEC,"textTransform":"uppercase","margin":"0 0 6px"}),
                html.Div(id="estado-btns",style={"display":"flex","flexWrap":"wrap"}),
            ]),
            html.Div([
                html.P("Período",style={"fontSize":"11px","color":TEXT_SEC,"textTransform":"uppercase","margin":"0 0 6px"}),
            ...
            html.Div([
                html.P("Variable Global",style={"fontSize":"11px","color":TEXT_SEC,"textTransform":"uppercase","margin":"0 0 6px"}),
    ...
    html.Div(style={"display":"grid","gridTemplateColumns":"1fr 1fr","gap":"14px","marginBottom":"14px"},children=[
        ...
        html.Div(style=STYLE_CARD,children=[
            ...
            html.Div(style={"display":"flex", "gap":"10px", "margin":"10px 0 0 14px", "alignItems":"center"}, children=[
                ...
                dcc.Dropdown(id="scatter-y", options=[{"label":l, "value":v} for v,l in VARS_DEF], value="empleo", clearable=False, style={"width":"130px","fontSize":"11px"}),
                dcc.Checklist(id="scatter-trend", options=[{"label":" Línea de Tendencia", "value":True}], value=[], style={"fontSize":"11px", "marginLeft":"10px"})
            ]),
    ...
    html.Div(style=STYLE_CARD,children=[
        ...
        html.Div(id="tabla-panel",style={"overflowX":"auto","maxHeight":"300px","overflowY":"auto"}),
    ]),
    ...
    html.Div(style={"margin":"28px 0 10px"},children=[
        html.H2("Módulo Econométrico (Modelo de Crecimiento)",style={"fontSize":"18px","fontWeight":"500","color":AZUL_OSCURO,"margin":"0 0 4px"}),
        html.P("PanelOLS · Efectos Fijos de entidad y tiempo · Tasas de Variación Trimestral (Δ%)",style={"fontSize":"12px","color":TEXT_SEC,"margin":"0"}),
    ]),
    ...
    html.Div(style={**STYLE_CARD,"background":"#EEF4FB","border":f"1px solid {AZUL_CLARO}44"},children=[
        ...
        html.Div(style={"marginTop":"10px", "display":"flex", "alignItems":"center", "gap":"15px"}, children=[
            ...
            dcc.Checklist(id="eco-vars", 
                          options=[{"label":" IED   ", "value":"IED"}, {"label":" Actividad Ind.   ", "value":"ActInd"}, {"label":" Exportaciones", "value":"Exportaciones"}], 
                          value=["IED", "ActInd", "Exportaciones"], inline=True, style={"fontSize":"12px", "color":AZUL_OSCURO})
        ])
    ]),

    html.Div(style={"display":"grid","gridTemplateColumns":"1fr 1fr","gap":"14px","marginBottom":"14px"},children=[
        html.Div(style=STYLE_CARD,children=[
            html.P("Coeficientes (Elasticidades)",style=STYLE_SEC_HDR),
            html.P("*** p<0.01  ** p<0.05  * p<0.1 · Errores clusterizados",style=STYLE_SUB),
...
@app.callback(
    ...
    Input("year-to","value"),
    Input("omitir-2020", "value"),
    Input("tipo-grafica", "value"),
    Input("scatter-x", "value"),
    Input("scatter-y", "value"),
    Input("scatter-trend", "value"),
    Input("eco-vars", "value"),
)
def update_all(estados, variable, yr_from, yr_to, omitir_2020, tipo_graf, scat_x, scat_y, scat_trend, eco_vars):
    panel, nacional, geojson = get_datos()
    ...
    metrics=[
        ("Peso Nacional", f"{peso_nac:.1f}%", "del empleo mfr. total"),
        ("Empleo prom.", f"{emp_bajio:,}", "personas en el Bajío"),
    ...
    metric_cards=[html.Div(style=STYLE_METRIC,children=[
        ...
        html.P(v,style={"fontSize":"18px","fontWeight":"500","color":TEXT_PRIM if l!="Peso Nacional" else AZUL_OSCURO,"margin":"0","lineHeight":"1.1"}),
    ...
    for c_var in eco["coefs"].index:
        coef = eco["coefs"].get(c_var, float("nan"))
        pval = eco["pvals"].get(c_var, float("nan"))
        ...
        filas_r.append(html.Tr([
            html.Td(var_names.get(c_var, c_var),style={"padding":"6px 10px","fontSize":"12px","fontWeight":"500"}),
            html.Td(f"{coef:,.3f}{stars}",style={"padding":"6px 10px","fontSize":"12px","textAlign":"right"}),
    ...
    # Robustez con Tooltips
    dw = eco["dw"]
    dw_txt = "Sin autocorrelación" if 1.5<dw<2.5 else ("Autocorrelación" if dw<1.5 else "Autocorrelación neg.")
    ...
    robustez = html.Div([
        html.P("Durbin-Watson (Auto-correlación)", title="Ideal entre 1.5 y 2.5. Indica si los errores del modelo están relacionados a través del tiempo.", style={"fontSize":"11px","color":TEXT_SEC,"margin":"0 0 4px","padding":"0 10px","cursor":"help", "borderBottom":"1px dashed #CCC", "display":"inline-block"}),
        ...
        html.P("VIF (Factor de Inflación de Varianza)", title="Valores > 5 indican Multicolinealidad. El modelo no puede separar el efecto porque las variables son casi idénticas.", style={"fontSize":"11px","color":TEXT_SEC,"margin":"0 0 4px","padding":"0 10px","cursor":"help", "borderBottom":"1px dashed #CCC", "display":"inline-block"}),
        html.Div(vif_items),
    ])
